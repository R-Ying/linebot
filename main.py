...
class yolop():
    def __init__(self, confThreshold=0.25, nmsThreshold=0.5, objThreshold=0.45):
        with open('bdd100k.names', 'rt') as f:
            self.classes = f.read().rstrip('\n').split('\n')   ###這個是在bdd100k資料集上訓練的模型做opencv部署的，如果在自己的資料集上訓練出的模型做opencv部署，需要修改self.classes
    ...
    def detect(self, srcimg):
        ...
        road_pixels = np.sum(seg_id == 1)
        total_pixels = srcimg.shape[0] * srcimg.shape[1]
        road_area_ratio = road_pixels / total_pixels
        if road_area_ratio >= 0.08:
...
if __name__ == "__main__":
    ...
    imgName = sys.stdin.readline().strip()
    srcimg = cv2.imread(imgName)
    outimg = yolonet.detect(srcimg)
    green_pixels = np.where((outimg[:, :, 0] == 0) & (outimg[:, :, 1] == 255) & (outimg[:, :, 2] == 0))
    if len(green_pixels[0]) > 0:  # 如果綠色像素的數量大於0，則存在綠色區塊
        processed_img = Image.fromarray(outimg)
        # processed_img.save("processed_img.png")
        print("Success")
    else:
        if os.path.exists("processed_img.png"):
